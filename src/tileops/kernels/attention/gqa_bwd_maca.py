...
def _flashattn_bwd_postprocess_kernel_maca(
    batch: int, heads: int, seq_len: int, dim: int, dtype: str = "float16"
) -> Callable:
    accum_dtype = "float"
...
class GQABwdMACAKernel(Kernel):
    ...
    def __init__(
        ...
        self.kernel = _gqa_bwd_kernel_maca(
            self.batch,
            self.heads,
            self.heads_kv,
            self.seq_len,
